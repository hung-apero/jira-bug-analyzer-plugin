...
| Source-of-truth capture (spec/figma/external — background, non-blocking) | **Lead** | once / phase | ask mandatory spec/figma, then background-capture digest |
...
- **Once per round (`--auto` only):** **acquire the device lock** (`~/.claude/jira-bug-analyzer/device-locks/<serial>.lock`) → install + launch the **round branch APK** (combined commits, appDev/debug — reuse the round build, don't rebuild) → run **the Phase 5 adb self-verify (`adb`, NOT mobile-mcp)** driving **every** ticket's repro + edge cases + screenshots in that **single** session → **release the lock**.
